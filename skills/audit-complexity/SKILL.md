...
Distillation depth, in typical yield order. These are high-yield patterns, not an exhaustive checklist: skip inapplicable categories, split or combine them when the code calls for it, and pursue other behavior-preserving simplifications supported by the same lens.
