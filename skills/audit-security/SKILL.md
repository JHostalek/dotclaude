...
Probe each category explicitly against the actual stack, assets, actors, and trust boundaries.
...
Filter by tracing each candidate sink to a concrete attack path from untrusted source to sink. Drop: server-controlled config (env vars, constants, `settings.*`), framework-mitigated sinks (React `{x}`, parameterized ORM, prepared statements), UUID identifiers, client-side-only validation when a server check also exists, and any candidate whose path stops at "could be vulnerable if".
