...
Probe each category explicitly against the actual stack, assets, actors, and trust boundaries. Required baseline coverage; skip one only when the scoped system cannot expose it.
...
Collect every candidate sink the probes surface, then make one pass over the collected list tracing each to a concrete attack path from untrusted source to sink. Drop in that pass, not before: server-controlled config (env vars, constants, `settings.*`), framework-mitigated sinks (React `{x}`, parameterized ORM, prepared statements), UUID identifiers, client-side-only validation when a server check also exists, and any candidate whose path stops at "could be vulnerable if".
