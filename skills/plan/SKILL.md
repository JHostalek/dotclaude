...
Create the implementation plan the executor will follow.

## Core Constraint

The executor starts with an empty context window. They get this plan and the codebase — nothing else. No conversation history, no shared understanding. Every ambiguity becomes a wrong guess they make confidently.

Match plan depth to task complexity. A 3-file bug fix needs a paragraph, not a document. The plan is the minimum artifact that prevents the executor from going wrong — no more, no less.

## Exploration

Two dimensions, run in parallel — spawn read-only exploration teammates when the surface is wide:

- **Codebase** — existing implementations, patterns, conventions, similar features. Reference precedent so the executor follows established patterns rather than inventing.
- **External** — web search. How others solved this. Libraries, vendor docs, open-source implementations.

Stop when you can articulate *why* one approach beats alternatives. If you can't articulate the tradeoffs, you haven't explored enough. If you're still reading files after the picture is clear, you've over-explored.
...
Name concrete files and functions in every step. Describe *what* changes and *why*. Link to precedent for the pattern to follow. Leave *how* to the executor — they need goals and constraints, not pseudocode.

Two litmus tests:
- "Could the executor start in the wrong file or with the wrong approach?" → be more specific.
- "Could the executor implement this without reading my plan?" → the step is too detailed.

Anti-pattern — narration disguised as steps:

    - [ ] Open auth.ts and look at the session handler
    - [ ] Think about whether the token refresh logic makes sense
    - [ ] Fix any bugs you find

Calibrated — named surface, linked precedent, mechanical verification:

    - [ ] `src/auth/session.ts:refreshToken` — replace silent-retry loop
          with the exponential-backoff pattern from
          `src/api/client.ts:retryWithBackoff`.
          Verify: `pnpm test auth.test.ts -t "refresh"` passes.
...
To update an existing plan, re-read it, diff against new requirements, revise in place.
