...
before you ship. review what you built, refactor what's messy, distill what's bloated, audit what's stale.
...
| `test-audit` | find redundant/weak tests, cover uncovered code |
| `comment-audit` | strip stale or bloated comments and docstrings |
| `silent-failures` | hunt swallowed errors and defaults masking bugs |
...
| Code audit suite | Yes (test-audit, comment-audit, silent-failures) | No | No |
...
Three things: (1) agent team orchestration — skills like `orch`, `qual`, `judge`, `refactor`, and `design` spawn multiple agents working in parallel; (2) adversarial and audit tooling — `sparring` pressure-tests proposals before commitment, while `test-audit`, `comment-audit`, and `silent-failures` find weak tests, stale prose, and swallowed errors; (3) depth — each skill encodes specific methodologies rather than generic checklists (e.g., seo-geo includes 9 DataForSEO scripts and 6 reference guides based on Princeton GEO research).
